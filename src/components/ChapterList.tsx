...
const ChapterList = ({ className = '' }: ChapterListProps) => {
  ...
  // Enhanced decoration styles for each card
  const getCardStyle = (index: number) => {
    const styles = [
      "border-t-4 border-t-saffron-400 dark:border-t-saffron-600",
      "border-l-4 border-l-saffron-400 dark:border-l-saffron-600",
      "border-r-4 border-r-saffron-400 dark:border-r-saffron-600",
      "border-b-4 border-b-saffron-400 dark:border-b-saffron-600",
  ...
  return (
    <div ref={containerRef} className={`mt-8 ${className}`}>
      <h2 className="text-2xl md:text-3xl font-serif font-medium text-earth-900 dark:text-earth-50 mb-8 text-center relative">
        <span className="relative z-10 inline-flex items-center">
          <Sparkles size={18} className="text-saffron-500 dark:text-saffron-400 mr-2 opacity-70" />
          Explore Chapters
          <Sparkles size={18} className="text-saffron-500 dark:text-saffron-400 ml-2 opacity-70" />
        </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {chapters.map((chapter, index) => (
          ...
          >
            {/* Visual embellishment with enhanced styling */}
            <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden opacity-10">
              <div className="absolute top-0 right-0 w-8 h-8 rounded-full bg-gradient-to-br from-saffron-400 to-saffron-300"></div>
            </div>
            
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-medium bg-saffron-100 text-saffron-800 dark:bg-saffron-900/30 dark:text-saffron-300 rounded-full px-3 py-1 flex items-center shadow-sm">
            ...
            <div className="flex justify-end">
              <span className="text-saffron-600 dark:text-saffron-400 text-sm inline-flex items-center font-medium group bg-saffron-50/50 dark:bg-saffron-900/20 px-3 py-1 rounded-full">
                Read Chapter 
